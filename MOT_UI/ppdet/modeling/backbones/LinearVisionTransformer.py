# 引用地址：https://aistudio.baidu.com/aistudio/projectdetail/2104496?channelType=0&channel=0
# 作者：仰世而来丶
# 引用日期：2021/6/29
...
class LinearBlock(nn.Layer):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # First stage
        self.mlp1 = Mlp(in_features=dim, hidden_features=int(
...
class LearnedPositionalEncoding(nn.Layer):
    ...
    def forward(self, x):
        ...
        if H == self.height and W == self.width:
            ...
            pos_embed = F.interpolate(self.pos_embed, size=[
...
class LinearVisionTransformer(nn.Layer):
    ...
    def forward_features(self, x):
        ...
        pos_embed = paddle.concat(
            [cls_pos_embed, pos_embed], axis=1)  # class pos emb
        x = x + pos_embed
        x = self.pos_drop(x)
