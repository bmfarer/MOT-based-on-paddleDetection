...
def plot_tracking(image,
    ...
    cv2.putText(
        ...
        text_scale, (0, 0, 255),
        thickness=2)
    if not tlwhs: # 判断tlwhs是否为空
        return im
    tlwhs_scores=np.insert(tlwhs,4,values=scores,axis=1)
    for i, tlwh_score in enumerate(tlwhs_scores):
        
        x1, y1, w, h,score = tlwh_score
